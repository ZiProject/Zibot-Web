...
export const isDiscordActivity = (): boolean => {
	...
	const isDiscordOrigin = Boolean(window.location.ancestorOrigins?.[0]?.includes("discord.com"));
...
export const getApiBase = (): string => {
	...
	console.log(`[DiscordUtils] getApiBase: isDiscordActivity=${isActivity} -> Result=${result}`);
	return result;
};
...
export const apiUrl = (path: string): string => {
	...
	console.log(`[DiscordUtils] apiUrl (path: "${path}"): isDiscordActivity=${isActivity} -> Result=${result}`);
	return result;
};
...
export const getWsUrl = (): string => {
	...
	if (isDiscordActivity()) {
		console.log("[DiscordUtils] getWsUrl: inside Discord Activity -> Result=/api/ws");
		return "/api/ws";
	}

	const base = import.meta.env.VITE_BotAPI || "https://api.ziji.best";
	const result = base.replace(/^https:\/\//, "wss://").replace(/^http:\/\//, "ws://") + "/ws";

	console.log(`[DiscordUtils] getWsUrl: outside Discord Activity -> Base=${base} -> Result=${result}`);
	return result;
};
...
export async function getDiscordClientId() {
	...
	if (_clientId) {
		console.log(`[DiscordUtils] getDiscordClientId: Returning cached _clientId=${_clientId}`);
		return _clientId;
	}

	const targetUrl = apiUrl("");
	console.log(`[DiscordUtils] getDiscordClientId: Fetching client ID from ${targetUrl}`);
	...
	console.log(`[DiscordUtils] getDiscordClientId: Fetch response status=${res.status} (${res.statusText})`);

	if (!res.ok) {
		console.error("[DiscordUtils] getDiscordClientId: Cannot fetch Bot API (response not ok)");
	...
	console.log(`[DiscordUtils] getDiscordClientId: Updated cached _clientId=${_clientId}`);
...
export async function loginViaActivity(): Promise<string | null> {
	...
	if (!isDiscordActivity()) {
		console.warn("[DiscordUtils] loginViaActivity: Not running inside Discord Activity. Aborting.");
		return null;
	}

	console.log("[DiscordUtils] loginViaActivity: Running inside Discord Activity. Retrieving Client ID...");
	const clientId = await getDiscordClientId();

	if (!clientId) {
		console.error("[DiscordUtils] loginViaActivity: Không lấy được Discord Client ID");
		return null;
	}
	console.log(`[DiscordUtils] loginViaActivity: Using Client ID=${clientId}`);

	console.log("[DiscordUtils] loginViaActivity: Dynamically importing @discord/embedded-app-sdk...");
	const { DiscordSDK } = await import("@discord/embedded-app-sdk");

	console.log("[DiscordUtils] loginViaActivity: Initializing DiscordSDK instance...");
	const sdk = new DiscordSDK(clientId);
	_sdk = sdk;

	console.log("[DiscordUtils] loginViaActivity: Waiting for sdk.ready()...");
	await sdk.ready();
	console.log("[DiscordUtils] loginViaActivity: sdk.ready() resolved successfully.");
	...
	console.log("[DiscordUtils] loginViaActivity: Calling sdk.commands.authorize with options:", authOptions);

	const { code } = await sdk.commands.authorize(authOptions);
	console.log(`[DiscordUtils] loginViaActivity: Received authorization code=${code}`);

	const tokenAuthUrl = apiUrl("/auth/token");
	console.log(`[DiscordUtils] loginViaActivity: Exchanging code for token at ${tokenAuthUrl}`);
	...
	console.log(`[DiscordUtils] loginViaActivity: Token exchange response status=${res.status}`);

	if (!res.ok) {
		const errorText = await res.text();
		console.error("[DiscordUtils] loginViaActivity: Token exchange thất bại", errorText);
		return null;
	}

	const { token } = await res.json();
	console.log(`[DiscordUtils] loginViaActivity: Token received successfully (Length: ${token?.length || 0})`);

	localStorage.setItem("ziji-token", token);
	console.log("[DiscordUtils] loginViaActivity: Saved token to localStorage ('ziji-token')");

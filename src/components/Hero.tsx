...
import { Sparkles, MessageSquare, Zap, Shield, ChevronDown } from "lucide-react";
		...
		<section className='relative pt-32 pb-20 px-6 overflow-hidden'>
			...
			<div className='max-w-7xl mx-auto flex flex-col items-center text-center relative z-10'>
				...
				<motion.div
					...
					<a
						href={
							botInfo?.inviteUrl || "https://discord.com/oauth2/authorize?client_id=1005716197259612193&permissions=8&scope=bot"
						}
						target='_blank'
						rel='noreferrer'
					...
					{botInfo?.playerNetClient?.length && (
						<div className='relative'>
							<button
								...
								<ChevronDown
									size={18}
									className={`transition-transform ${openBots ? "rotate-180" : ""}`}
								/>
							</button>
									...
									{botInfo.playerNetClient.map((bot) => (
										<a
											...
											<div className='flex flex-col text-left'>
												...
												<span className='text-xs text-zinc-400'>{bot.clientId}</span>
											</div>
										</a>
									))}
								</div>

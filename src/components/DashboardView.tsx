...
export function DashboardView({ botInfo, loading, error }: DashboardViewProps) {
  ...
  const addAutoResponder = async () => {
    if (
      !token ||
      !selectedGuild ||
      !newResponder.trigger ||
      !newResponder.response
    )
      return;
    setSaving(true);
    try {
          ...
          <h2 className="text-5xl font-extrabold tracking-tighter">
            {/* {t("dashboardTitle")
              ...
              .map((word, i, arr) => (
                <span
                  key={i}
                  className={i === arr.length - 1 ? "text-discord" : ""}
                >
                  {word}{" "}
                </span>
              ))} */}
            <span key={1} className="text-discord">
              {botInfo?.clientName}
            </span>
            dashboard
          </h2>
      ...
      <div className="min-h-[400px]">
              ...
              >
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-zinc-600 font-mono text-sm">
                      {user ? `#${user.id.slice(-4)}` : "#guest_1337"}
                    </p>
                  </div>
                </div>
                <div className="grid grid-cols-1 gap-4">
                  <div className="flex justify-between items-center p-5 bg-white/[0.02] border border-white/5 rounded-2xl">
                    ...
                    <span className="font-mono text-xl font-bold text-discord">
                      {user ? user.level : 0}
                    </span>
                  </div>
                  <div className="flex justify-between items-center p-5 bg-white/[0.02] border border-white/5 rounded-2xl">
              ...
              >
                ...
                <div className="space-y-8 flex-grow">
                  <div className="p-8 bg-white/[0.02] border border-white/5 rounded-[2rem]">
                    <p className="text-zinc-600 font-bold uppercase text-[10px] tracking-widest mb-4">
                      {t("apiMsg")}
                    </p>
        ...
        {activeTab === "settings" && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="glass rounded-[2.5rem] p-10 max-w-3xl mx-auto"
          >
            <h3 className="font-extrabold text-2xl tracking-tight mb-8">
              {t("userSettings")}
            </h3>
              ...
              <div className="space-y-8">
                <div className="space-y-4">
                  <label className="flex items-center gap-3 text-sm font-bold text-zinc-400 uppercase tracking-widest">
                    <Languages className="w-4 h-4 text-discord" />{" "}
                    {t("language")}
                  </label>
                  <div className="flex gap-4">
                    {["vi", "en", "ja"].map((l) => (
                      <button
                        key={l}
                        onClick={() =>
                          setUserSettings({ ...userSettings, lang: l })
                        }
                ...
                <div className="space-y-4">
                  <label className="flex items-center gap-3 text-sm font-bold text-zinc-400 uppercase tracking-widest">
                    <Volume2 className="w-4 h-4 text-discord" /> {t("volume")} (
                    {userSettings.volume}%)
                  </label>
                  <input
                    title="volume"
          ...
          >
            <div className="flex items-center justify-between px-2">
              <h3 className="font-extrabold text-2xl tracking-tight">
                {selectedGuild ? selectedGuild.name : t("guildManagement")}
              </h3>
              {selectedGuild && (
                <button
                  onClick={() => {
            ...
            {!selectedGuild ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {guilds.map((guild) => (
                  <div
                    key={guild.id}
                    className="glass rounded-[2rem] p-6 hover:bg-white/[0.05] transition-all group"
                  >
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <div className="flex-grow overflow-hidden">
                        <h4 className="font-black text-lg truncate group-hover:text-discord transition-colors">
                          {guild.name}
                        </h4>
                  ...
                  <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <div className="flex flex-col gap-4 p-6 bg-white/[0.02] border border-white/5 rounded-3xl">
                      <div className="flex items-center justify-between">
                          ...
                          onClick={() =>
                            setGuildSettings({
                              ...
                              autoRole: {
                                ...
                                enabled: !guildSettings.autoRole?.enabled,
                              },
                            })
                          }
                        ...
                        >
                          <div
                            className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-white transition-transform ${guildSettings.autoRole?.enabled ? "translate-x-6" : ""}`}
                          />
                        </button>
                      </div>
                      {guildSettings.autoRole?.enabled && (
                        <div className="mt-4">
                          ...
                          <input
                            ...
                            type="text"
                            value={
                              guildSettings.autoRole.roleIds?.join(", ") || ""
                            }
                            onChange={(e) =>
                              setGuildSettings({
                                ...guildSettings,
                    ...
                    <div className="flex flex-col gap-4 p-6 bg-white/[0.02] border border-white/5 rounded-3xl">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          onClick={() =>
                            setGuildSettings({
                              ...guildSettings,
                              joinToCreate: {
                                ...guildSettings.joinToCreate,
                                enabled: !guildSettings.joinToCreate?.enabled,
                              },
                            })
                          }
                        ...
                        >
                          <div
                            className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-white transition-transform ${guildSettings.joinToCreate?.enabled ? "translate-x-6" : ""}`}
                          />
                        </button>
                      </div>
                    </div>
                        ...
                        {autoResponders.map((ar, idx) => (
                          ...
                          >
                            ...
                            <div className="text-right">
                              <p className="text-[10px] font-bold text-discord uppercase tracking-widest">
                                Response
                              </p>
                              <p className="font-mono text-xs opacity-70 truncate max-w-[200px]">
                                {ar.response}
                              </p>
                            </div>
                          </div>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <div className="p-4 bg-white/5 border border-white/10 rounded-2xl group-hover:glow transition-all">
          {icon}
        </div>
      ...
      <p
        className={`text-3xl font-black tracking-tight truncate ${accent || "text-white"}`}
      >
        {value}
      </p>
      {subValue && (
